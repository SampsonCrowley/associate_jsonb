...
RSpec.shared_examples ":has_many association" do |store_type: :regular|
  ...
  describe "#collection.destroy" do
    ...
    it "sets the foreign_key to NULL on removed association" do
  ...
  describe "#collection.delete" do
    ...
    it "sets the foreign_key to NULL on removed association", if: store_type.eql?(:regular) do
      expect(child_to_remove.reload.send(foreign_key)).to be_nil
    end

    it "sets the coalesced store_key/foreign_id to NULL in JSONB column on removed association", if: store_type.eql?(:jsonb) do
      expect(child_to_remove.reload.send(store)[foreign_store_key.to_s]).to be_nil
      expect(child_to_remove.reload.send(store)[foreign_store_key.to_sym]).to be_nil
    end
  ...
  describe "#collection.build" do
    ...
    it "sets the foreign_key in store column", if: store_type.eql?(:jsonb) do
      expect(built_child.send(store)[foreign_store_key.to_s]).to eq(parent.id)
    end
  end
  ...
  describe "#collection.create" do
    ...
    it "sets the foreign_key in store column", if: store_type.eql?(:jsonb) do
      expect(
        created_child.reload.send(store)[foreign_store_key]
      ).to eq(parent.id)
    end
  end
...
RSpec.describe ":has_many" do
  context "regular association" do
    include_examples ":has_many association" do
      ...
      let(:foreign_store_key) { "user_id" }
      let(:parent) { create :user }

      let(:children) do
        create_list child_factory_name, 3, foreign_store_key => parent.id
      end
    end
  end
  ...
  context "association with :store option set on child models" do
    let(:children) do
      create_list child_factory_name, 3,
                  store => { foreign_store_key => parent.id }
    end
    ...
    context "with non-default :options" do
      context "(foreign_key)" do
        include_examples ":has_many association", store_type: :jsonb do
          ...
          let(:foreign_store_key) { "supplier_id" }
      ...
      context "(foreign_key, store_key)" do
        include_examples ":has_many association", store_type: :jsonb do
          ...
          let(:foreign_store_key) { "label_user" }
          let(:parent_association_name) { :user }
          let(:parent) { create :user }
        end
      end
    end
